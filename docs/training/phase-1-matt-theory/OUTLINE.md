...
| 说法 | 含义 | 仓库里可能见到 |
| ---- | ---- | -------------- |
| **现状**（As-Is） | 系统现在实际在做什么 | `docs/prd/as-is/` |
| **目标态**（To-Be） | 我们打算改成什么样 | 模块化 PRD / 迁移计划 |
| **缺口** | 现状与目标态之间差什么 | gap / 差距矩阵 |
| **待办清单** | 可以实现的任务列表 | backlog |

口播尽量用「现状 / 目标态」；路径名和命令保持英文不动。
...
flowchart TB
  ...
  S1 --> P1["写需求 → 挑战门 → 循环实现"]
  S2 --> P2["摸清现状 → 补目标态 → 过门再续跑"]
  S3 --> P3["逆向现状 → 表征基线 → 再定改造"]
...
| 步骤 | 做什么 | 关键产物 |
| ---- | ------ | -------- |
| 1 | 摸清现状：哪些已交付、哪些半成品、代码落在哪 | 现状对照表（可局部写现状文档） |
| 2 | 对已有行为钉住反馈（表征测试，或关键路径手测清单） | 不敢轻易改坏的安全网 |
| 3 | 只为下一批变更写清目标态（缺口 + 范围） | 模块化目标态 / 续接计划 |
...
2. 把逆向出来的现状文档直接当实现待办：把「现在怎样」当成「要做成怎样」  
...
| 4 | 再写目标态 / 迁移计划（写目标，不写现状复述） | 改造待办 |
| 5 | 挑战门 → 循环实现 | 与新品后半段汇合 |

编排入口：`/prd-reverse` →（表征）→ 目标态 → `/prd-grill` → `/prd-build-loop`  
硬规矩：只有现状文档，不能进 build-loop。
...
| 先产出 | 模块化目标态 PRD | 现状对照 + 下一批目标态 | 现状文档 + 忠实度 |
| 过门 | 挑战门 | 挑战门（针对变更） | 逆向签核 + 挑战门 |
| 禁忌 | 跳过对齐直接写码 | 当白板重写 | 把现状文档当待办 |
